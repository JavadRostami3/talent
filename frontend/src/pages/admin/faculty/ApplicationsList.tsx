...
const FacultyApplicationsList = () => {
  ...
  const [reviewStatusFilter, setReviewStatusFilter] = useState<'COMPLETED' | 'PENDING' | ''>('');
  ...
  const fetchApplications = async () => {
    ...
    try {
      const filters: any = {
        page,
        page_size: pageSize,
        faculty_review_pending: reviewStatusFilter === 'PENDING' ? 'true' : undefined,
        faculty_review_completed: reviewStatusFilter === 'COMPLETED' ? 'true' : undefined,
      };
  ...
  const handleExport = async () => {
    try {
      const filters: any = {};
      if (statusFilter) filters.status = statusFilter;
      if (reviewStatusFilter === 'PENDING') filters.faculty_review_pending = 'true';
      if (reviewStatusFilter === 'COMPLETED') filters.faculty_review_completed = 'true';
      if (roundTypeFilter) filters.round_type = roundTypeFilter;
  ...
  const getReviewStatusBadge = (app: Application) => {
    if (app.faculty_review_completed) {
      return (
    ...
    if (app.status === 'UNDER_FACULTY_REVIEW') {
      return (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">بررسی تخصصی درخواست‌ها</h1>
          <p className="text-muted-foreground">مدیریت و بررسی تخصصی پرونده‌ها</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as ApplicationStatus)}>
                ...
                <SelectContent>
                  <SelectItem value="">همه</SelectItem>
                  <SelectItem value="APPROVED_BY_UNIVERSITY">تایید دانشگاه</SelectItem>
                  <SelectItem value="UNDER_FACULTY_REVIEW">بررسی دانشکده</SelectItem>
                  <SelectItem value="FACULTY_REVIEW_COMPLETED">بررسی تکمیل</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={reviewStatusFilter} onValueChange={(v) => setReviewStatusFilter(v as 'COMPLETED' | 'PENDING' | '')}>
                <SelectTrigger>
                  <SelectValue placeholder="همه" />
                </SelectTrigger>
                <SelectContent>
                  ...
                  <SelectItem value="COMPLETED">بررسی تکمیل</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
                ...
                <TableBody>
                  {applications.map((app) => (
                    <TableRow key={app.id}>
                      ...
                      <TableCell>{getReviewStatusBadge(app)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-left">
                        <Button
                          size="sm"
                          onClick={() => navigate(`/admin/faculty/applications/${app.id}`)}
                        >
...
export default FacultyApplicationsList;
