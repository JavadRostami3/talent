...
import AdvancedFilters, { ApplicationFilterValues } from '@/components/features/AdvancedFilters';
...
const MastersTalentedApplicants = () => {
  ...
  const [activeFilters, setActiveFilters] = useState<ApplicationFilterValues>({});
  const navigate = useNavigate();
  const { adminProfile } = useOutletContext<{ adminProfile: AdminProfile }>() || {};

  const fetchApplications = async (filters: ApplicationFilterValues = {}) => {
    setLoading(true);
    try {
      const params: any = { round_type: 'MA_TALENT' };
      ...
      if (filters.rank_status) params.rank_status = filters.rank_status;
  ...
  const handleFilterChange = (filters: ApplicationFilterValues) => {
    setActiveFilters(filters);
    fetchApplications(filters);
  };

  // Transform faculties for filter component
  const facultyOptions = adminProfile?.faculties?.map(f => ({
    label: f.name,
    value: String(f.id),
  })) || [];
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        <h1 className="text-center text-3xl font-bold text-foreground mb-2">
          پرونده‌های استعدادهای درخشان (کارشناسی ارشد)
        </h1>
        <p className="text-center text-muted-foreground">
          لیست متقاضیان استعدادهای درخشان در مقطع کارشناسی ارشد و وضعیت بررسی پرونده‌های آنان
        </p>
      </div>
